...
class StoreDetailTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.get(pk=self.store.pk)
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
    ...
    def test_get_store(self):
        ...
        self.assertEqual(self.store.address.country, response.data["address"]["country"])
    ...
    def test_get_store_with_wrong_id_raises_not_found(self):
        response = self.client.get(
            reverse("store-detail", args=["11111111-1111-4111-a111-000000000002"]), format="json"
        )
    ...
    def test_update_store(self):
        ...
        response = self.client.patch(
            reverse("store-detail", args=[self.store.pk]), data=store_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_store_with_user_without_permission_raises_forbidden(self):
        user2 = User.objects.get(pk="44444444-4444-4444-a444-000000000004")
    ...
    def test_update_store_with_wrong_id_raises_not_found(self):
        ...
        response = self.client.patch(
            reverse("store-detail", args=["11111111-1111-4111-a111-000000000002"]), data=store_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_delete_store(self):
        ...
        self.assertFalse(
            Store.objects.filter(pk=self.store.pk).exists()
        )
    
    def test_delete_store_with_user_without_permission_raises_forbidden(self):
        user2 = User.objects.get(pk="11111111-1111-4111-a111-000000000001")
        self.client.force_authenticate(user=user2)
        response = self.client.delete(
            reverse("store-detail", args=[self.store.pk]), format="json"
        )
